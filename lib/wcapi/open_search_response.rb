  ...
  class OpenSearchResponse 
    ...
    def parse_rss(xml)
      ...
      nodes.each do |item|
        ...
        if xpath_first(item, "dc:identifier") != nil
          xpath_all(item, "dc:identifier").each do |i|
            _isbn << xpath_get_text(i)
          end
        end
        ...
        @records << {:title => _title, :author => _author, :link => _link, 
            :id => _id, :citation => _citation, :summary => _summary, 
            :isbns => _isbn, :xml => item.to_s}
      end
    end
    ...
    def parse_atom(xml)
      ...
      nodes.each do |item|
        ...
        @records << {:title => _title, :author => _author, :link => _link, :id => _id, 
            :citation => _citation, :summary => _summary, 
            :isbns => _isbn, :xml => item.to_s}
      end
    end
  end
